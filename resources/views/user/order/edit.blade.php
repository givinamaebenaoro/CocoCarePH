    ...
    <form action="{{route('order.update',$order->id)}}" method="POST">
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          ...
          <option value="process" {{($order->status=='delivered'|| $order->status=="cancel") ? 'disabled' : ''}}  {{(($order->status=='process')? 'selected' : '')}}>Processing</option>@section('main-content')
          <div class="card">
            <h5 class="card-header">Order Edit</h5>
            <div class="card-body">
          ...
          @endpush
          <option value="cancel" {{($order->status=='delivered') ? 'disabled' : ''}}  {{(($order->status=='cancel')? 'selected' : '')}}>Cancel Order</option>
        </select>
      </div>
      <button type="submit" class="btn btn-primary">Update</button>
    </form>
